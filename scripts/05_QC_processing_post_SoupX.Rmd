...
strain_order <- factor(metadata$genotype, levels = unique(metadata$genotype))
...
```{r seurat_object}
prefix <- "../cellranger/"
suffix <- "/SoupX_set_fraction_20percent/"
...
dataObject.filtered$seurat_clusters <- dataObject.filtered$SCT_snn_res.0.8
...
# Determine the clusters for various resolutions
dataObject.integrated <- FindClusters(object = dataObject.integrated, resolution = 0.4)
...
```{r}
dataObject.filtered <- CellCycleScoring(dataObject.filtered, 
                                        s.features = m.s.genes$gene_name, 
                                        g2m.features = m.g2m.genes$gene_name, 
                                        set.ident = TRUE)

# Visualize the distribution of cell cycle markers across
RidgePlot(dataObject.filtered, features = c("Pcna", "Top2a", "Mcm6", "Mki67"), ncol = 2)

dataObject.filtered <- RunPCA(dataObject.filtered, features = c(m.s.genes$gene_name, m.g2m.genes$gene_name))
DimPlot(dataObject.filtered, reduction = "pca")
```
