...
group_no_sex <- paste0(time_order, ".", strain_order)
table(group_no_sex)
```
...
```{r glia_clean}
check_meta <- dataObject.glia.clean@meta.data
...
dataObject.glia.clean$group_no_sex <- group_no_sex[sample_match]
table(dataObject.glia.clean$group_no_sex) 

dataObject.glia.clean$genotype <- genotype_order[sample_match]
table(dataObject.glia.clean$genotype) 

dataObject.glia.clean$strain <- strain_order[sample_match]
table(dataObject.glia.clean$strain) 
...
dataObject.neurons.clean$group_no_sex <- group_no_sex[sample_match]
table(dataObject.neurons.clean$group_no_sex) 

dataObject.neurons.clean$genotype <- genotype_order[sample_match]
table(dataObject.neurons.clean$genotype) 

dataObject.neurons.clean$strain <- strain_order[sample_match]
table(dataObject.neurons.clean$strain) 
...
bGroup2 <- dataObject.glia.clean@meta.data %>%
  group_by(individual_clusters, group_no_sex) %>%
  dplyr::count() %>%
  group_by(individual_clusters) %>%
  dplyr::mutate(percent = 100*n/sum(n)) %>%
  ungroup() %>%
  ggplot(aes(x=individual_clusters,y=percent, fill=group_no_sex)) +
  geom_col() +
  ggtitle("Percentage of group per cluster")
bGroup2
```
...
path <- paste0("../results/nuclei_count/",treatment,
               "_percent_nuclei_per_cluster_group_no_sex_glia_clean")
bGroup2
saveToPDF(paste0(path, ".pdf"), width = 10, height = 4)
```
...
data <- as.data.frame(table(dataObject.glia.clean$group_no_sex))
colnames(data) <- c("group_no_sex","frequency")
ncells_group2 <- ggplot(data, aes(x = group_no_sex, y = frequency, fill = group_no_sex)) + 
  geom_col() +
  theme_classic() +
  geom_text(aes(label = frequency), 
            position=position_dodge(width=0.9), 
...
ncells_group2
path <- paste0("../results/nuclei_count/",treatment, 
               "_cells_per_group_no_sex_glia_clean")
saveToPDF(paste0(path, ".pdf"), width = 7, height = 4)
...
bGroup2 <- dataObject.neurons.clean@meta.data %>%
  group_by(individual_clusters, group_no_sex) %>%
  dplyr::count() %>%
  group_by(individual_clusters) %>%
  dplyr::mutate(percent = 100*n/sum(n)) %>%
...
path <- paste0("../results/nuclei_count/",treatment,
               "_percent_nuclei_per_cluster_group_no_sex_neurons_clean")
bGroup2
saveToPDF(paste0(path, ".pdf"), width = 6, height = 4)
```
...
# Visualize the number of cell counts per group
data <- as.data.frame(table(dataObject.neurons.clean$group_no_sex))
colnames(data) <- c("group","frequency")

ncells_group2 <- ggplot(data, aes(x = group, y = frequency, fill = group)) + 
  geom_col() +
  theme_classic() +
  geom_text(aes(label = frequency), 
            position=position_dodge(width=0.9), 
            vjust=-0.25) +
  scale_y_continuous(breaks = seq(0,25000, by = 1000), limits = c(0,25000)) +
  ggtitle("Filtered: nuclei count per group
neurons cell types only") +
  theme(legend.position =  "none") + 
  theme(axis.text.x = element_text(angle = 45, hjust=1))
...
ncells_group2
path <- paste0("../results/nuclei_count/",treatment, 
               "_cells_per_group_no_sex_neurons_clean")
saveToPDF(paste0(path, ".pdf"), width = 7, height = 4)
...
# Cell cycle 
```{r cellcycle}
dataObject <- CellCycleScoring(dataObject.clean, 
...
# sample before integration 
cellcyle_group2 <- dataObject@meta.data %>%
...
path <- paste0("../results/cellcycle/",treatment,
               "_percent_phase_group_no_sex")
cellcyle_group2
saveToPDF(paste0(path, ".pdf"), width = 10, height = 5)

info <- dataObject.clean@meta.data
p_65 <- subset(info, sample == "P_65")
```
