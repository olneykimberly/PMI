...
pseudo_bulk_meta <- dataObject.pseudo@meta.data
    ...
    pct_data <- calculate_percentages(filtered_counts, factor(pseudo_bulk_meta$strain))
    ...
    strain_output_path <- file.path(output_dir, paste0("all_celltypes", "_PS19_vs_WT_comparison_pseudobulk.txt"))
...
# Time
    ...
    # Calculate pct.1 and pct.2
    pct_data <- calculate_percentages(filtered_counts, factor(pseudo_bulk_meta$time))
    ...
    time_output_path <- file.path(output_dir, paste0("all_celltypes", "_3_vs_0_hours_comparison_pseudobulk.txt"))
    write.table(DEGs, file = time_output_path, sep = "\t", quote = FALSE, row.names = FALSE)
    rm(dds, res, DEGs, dataObject.pseudo, filtered_counts, filtered_genes, pct_data, pseudo_bulk_counts, pseudo_bulk_meta) # clean up 
...
for (cell_type in cell_types) {
    ...
    write.table(DEGs, file = time_output_path, sep = "\t", quote = FALSE, row.names = FALSE)
    rm(dds, res, DEGs, filtered_counts, filtered_genes, pct_data, pseudo_bulk_counts, pseudo_bulk_meta) # clean up 
