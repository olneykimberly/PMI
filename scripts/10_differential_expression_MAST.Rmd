...
## 1) PS19 vs WT
## 2) 3 hours vs Fresh
```{r DESeq_DE_all_samples_pseudobulk}
# Directory to save the results
output_dir <- "../results/DEGs/DESeq2_pseudobulk"

# pseudobulk the counts based on sample-strain-sex-time-celltype
dataObject.pseudo <- AggregateExpression(dataObject, assays = "RNA", return.seurat = T, group.by = c("sample", "strain", "sex", "time", "individual_clusters")) 
...
for (cell_type in cell_types) {
  ...
  df <- FindMarkers(dataObject.pseudo, ident.1 = paste(cell_type, "PS19", sep = "_"), ident.2 = paste(cell_type, "WT", sep = "_"), 
  ...
  strain_output_path <- file.path(output_dir, paste0(cell_type, "_PS19_vs_WT_comparison_pseudo_bulk.txt"))
  ...
  df <- FindMarkers(dataObject.pseudo, ident.1 = paste(cell_type, "3", sep = "_"), ident.2 = paste(cell_type, "0", sep = "_"), 
                                   test.use = "DESeq2", verbose = FALSE)
  df$percent_difference <- abs(df$pct.1 - df$pct.2)
  colnames(df)[c(3,4)] <- c("percent_3","percent_0")
  df$gene <- rownames(df)
  DEGs <- df[,c(7,1,2,5,3,4,6)]
  strain_output_path <- file.path(output_dir, paste0(cell_type, "_3_vs_0_hours_comparison_pseudo_bulk.txt"))
  write.table(DEGs, file = strain_output_path, sep = "\t", quote = FALSE, row.names = FALSE)
  rm(df, DEGs)
}
```
