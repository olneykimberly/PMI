...
if (tissue == "Brain" && file.exists("../../rObjects/cell_replicate_2_merged_h5.rds")) {
  PMI <- readRDS("../../rObjects/cell_replicate_2_merged_h5.rds")
  ...
  saveRDS(PMI, "../../rObjects/cell_replicate_2_merged_h5.rds")
...
# save vXist
vXist
path <- paste0("../../results/vlnPlots/",treatment,"_",tolower(tissue),"_percent_raw_Xist")
saveToPDF(paste0(path, ".pdf"), width = 6, height = 4)
saveToPNG(paste0(path, ".png"), width = 6, height = 4, unit = "in", res = 300)
dev.off()
# cleanup
...
counts.df <- FetchData(PMI.split$WT_0_hr, vars = goi)
...
```
## Filter by gene of interest expression
remove cells that do not express Xist
remove cells that do not express Malat1
remove cells that do express Ttr
```{r}
...
# which cells have a count for Malat1?
cellsGoi<- WhichCells(PMI.filtered, slot = 'counts', expression = Malat1 > 0 )

# percent xist in each cell 
PMI.filtered$percent.Malat1 <- PercentageFeatureSet(PMI.filtered, features = "Malat1")

vMalat1 <- VlnPlot(PMI.filtered,
              features = c("percent.Malat1"),
              group.by = 'sample',
              cols = sample_colors,
              pt.size = 0)
vMalat1
...
# remove cells less than 0 percent Xist expresison
Malat1.filtered <- subset(PMI.filtered,
                        subset = percent.Malat1 > 0)

# print cells removed
print(paste0(dim(PMI.filtered)[2] - dim(Malat1.filtered)[2]," cells removed"))
# print features removed
print(paste0(dim(PMI.filtered)[1] - dim(Malat1.filtered)[1], " features removed"))

remove(PMI.filtered)
# overwrite PMI.filtered
PMI.filtered <- Malat1.filtered



# which cells have a count for Ttr?
cellsGoi <- WhichCells(PMI.filtered, slot = 'counts', expression = Ttr > 0 )

# percent xist in each cell 
PMI.filtered$percent.Ttr <- PercentageFeatureSet(PMI.filtered, features = "Ttr")
...
# remove cells less than 0 percent Xist expresison
Ttr.filtered <- subset(PMI.filtered,
                        subset = percent.Ttr <= 0)

# print cells removed
print(paste0(dim(PMI.filtered)[2] - dim(Ttr.filtered)[2]," cells removed"))
# print features removed
print(paste0(dim(PMI.filtered)[1] - dim(Ttr.filtered)[1], " features removed"))

remove(PMI.filtered)
# overwrite PMI.filtered
PMI.filtered <- Ttr.filtered
...
```{r fx_convert_gene_names, eval=FALSE}
...
saveRDS(PMI.sct.merged, "PMI.sct.merged.rds")
PMI.sct.merged <- readRDS("PMI.sct.merged.rds")

PMI.sct.merged <- RunUMAP(PMI.sct.merged, dims = 1:15, reduction = "pca")

# Plot PCA
pca1 <- DimPlot(PMI.sct.merged,
        reduction = "umap",
      #  split.by = "treatment",
        group.by = "treatment",
...
pca3 <- DimPlot(PMI.sct.merged,
        reduction = "umap",
        group.by = "sample",
        shuffle = TRUE)
pca3
# nCount
FeaturePlot(PMI.sct.merged, 
            features = "Tmem19",
            pt.size = 0.4, 
            order = TRUE,
            min.cutoff = 'q10',
