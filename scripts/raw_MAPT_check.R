...
for (sample_id in sample_ids) {
  ...
  data_path <- paste0(base_path, "cellrangerMAPT/", sample_id, "/outs/filtered_feature_bc_matrix.h5")
  ...
  sex_linked_plot <- VlnPlot(seurat_obj, features = c("Mapt", "MAPT", "Ctsb"), ncol = 4, pt.size = 0.1) +
    plot_annotation(title = sample_id)
  
  # Save plot as PDF
  output_path <- paste0(base_path, "results/Mapt/", sample_id, "_violin.pdf")
