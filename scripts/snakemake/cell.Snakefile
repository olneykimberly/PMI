...
rule all:
        input:
               # expand("{sample}/",sample = config["sample_names"]),
                ...
                expand(config["cellbender_dir"]+"scRNA/{sample}_cellbender.h5", sample = config["sample_names"])
...
rule cellbender:
        output:
            filtered = (config["cellbender_dir"]+"scRNA/{sample}_cellbender.h5")
        params:
            id = lambda wildcards: config[wildcards.sample]["ID"],
        shell:
            """
            mkdir {output.filtered};
            sh cell_cellbender.sh {params.id};
